...
export default function ViewData() {
  ...
  const selectedLocation = reactLocation.state?.location;
  ...
  useEffect(() => {
    if (location) {
      axios
        .get("https://bmfh.medium-blog-clone.workers.dev/get-venues", {
          params: { location },
        })
        .then((res) => setData(res.data.reqVenues)) // Ensure the response structure matches
  ...
  const filteredData = data.filter((venue) => venue.location === location);

  return (
    <div>
      <div className="flex flex-col items-center my-5">
        <h2 className="text-2xl font-semibold py-4">Venues in <span className="underline">{selectedLocation}</span></h2>
        <div className="w-[60vw]">
          {filteredData.length > 0 ? (
            filteredData.map((venue: VenueData) => (
              <div
                key={venue.id}
              ...
              >
                <div>
                  <img
                    ...
                    src={convertBase64ToImageUrl(venue.image)}
                    alt="location image"
                  />
                </div>
